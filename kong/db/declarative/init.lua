...
do
  ...
  function declarative.load_into_cache_with_events(entities, meta, hash, hashes)
    ...
    if ok then
      ...
      local balancer_hash
      if hashes then
        if hashes.services ~= DECLARATIVE_EMPTY_CONFIG_HASH then
          if hashes.routes ~= DECLARATIVE_EMPTY_CONFIG_HASH then
            router_hash = md5(hashes.services .. hashes.routes)
          else
            router_hash = hashes.services
          end
        elseif hashes.routes ~= DECLARATIVE_EMPTY_CONFIG_HASH then
          router_hash = hashes.routes
        else
        ...
        if hashes.upstreams ~= DECLARATIVE_EMPTY_CONFIG_HASH then
          if hashes.targets ~= DECLARATIVE_EMPTY_CONFIG_HASH then
            balancer_hash = md5(hashes.upstreams .. hashes.targets)
          else
            balancer_hash = hashes.upstreams
          end
        elseif hashes.targets ~= DECLARATIVE_EMPTY_CONFIG_HASH then
          balancer_hash = hashes.targets
        else
